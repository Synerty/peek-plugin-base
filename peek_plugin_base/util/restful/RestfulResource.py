...
class _JsonResource(BasicResource):
    ...
    def _requestToTuple(self, request) -> TupleABC:
    ...
    @deferToThreadWrapWithLogger(logger)
    def _tupleToResponse(self, tuple_:TupleABC) -> bytes:
        dict_ = tupleToRestfulJsonDict()
    ...
    def _renderAsync(self, request):
        tupleIn = None
        try:
            tupleIn = yield self._parseRequest(request)
            ...
            logger.debug(f"Error while processing REST Tuple\n{tupleIn}\n{request.path}")
            logger.exception(e)
